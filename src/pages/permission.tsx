...
import { createPermission, deletePermissionById, getPermissions, modifiedPermission } from "../services/permissionService";
...
const PermissionPage: React.FC = () => {
    ...
    const currentInitialValues: Permission = editingPermission
        ? { url: editingPermission.url, method: editingPermission.method, model: editingPermission.model }
        : initialValues;
    ...
    const handleSubmit = async (values: Permission) => {
        ...
        if (!values.method?.trim()) { alert("El método es obligatorio"); return; }
        if (!values.model?.trim()) { alert("El modelo es obligatorio"); return; }
        if (!values.url?.trim()) { alert("La URL es obligatoria"); return; }
